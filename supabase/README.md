...
- `config/` — Supabase CLI project configuration, linking this repo to the `revnara-local`/`revnara-staging`/`revnara-prod` projects (Auth provider settings, Storage bucket policy, etc. — platform configuration Alembic has no concept of). `storage_buckets.sql` (Sprint 4) provisions the private `company-files` bucket used by `app/files/storage.py`. `rag_queues.sql` (Sprint 5) provisions the `document_tasks`/`embedding_tasks` Supabase Queues (pgmq) used by `backend/workers/`.

## Real project (Sprint 4 onward)

A real Supabase project (`nbnmlfsxtiivvvoxuhdr`, dev/local use) is connected as of Sprint 4 -- migrations, every `rls/*.sql` file, `config/storage_buckets.sql`, and (Sprint 5) `config/rag_queues.sql` have all been applied against it, and the whole request path (real Supabase Auth sign-in -> JWKS-verified JWT -> Postgres via a dedicated non-bypassrls `revnara_app` role -> RLS-enforced tenant isolation -> real Storage upload -> real document/embedding worker pipeline -> real pgvector similarity search) has been verified end to end against it, not just locally.

Things worth knowing about this project's setup that aren't obvious from the SQL files alone:
...
- **`vector` and `pgmq` (Sprint 5) are not available on a vanilla Postgres install at all** -- confirmed empirically against the local portable Postgres used for `backend/tests/rls/`, which has neither. Real Supabase Queue/vector work is therefore only verifiable against this real project, a stricter requirement than the RLS suite's "any real Postgres" -- see `docs/rag-pattern.md` and `backend/tests/rag/README.md`.
- **Queue creation (`pgmq.create()`) needs the `pgmq` extension owner's privilege**, not just schema access -- on this project, the `postgres` admin role enabled the extension first, so `config/rag_queues.sql` (which provisions the two queues) must be run by that role, not `revnara_app`. See `docs/rag-pattern.md` for why a fresh environment might not have this restriction at all.
