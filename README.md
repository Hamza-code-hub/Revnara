...
Sprint 1, 1.5 (Design System), 2 (Identity & Tenancy Backend), 3 (Authorization, RLS & Tenant Isolation Hardening), 4 (Company Brain Data Model), and 5 (Company Brain Retrieval / RAG) are complete. Cross-tenant row-level security, the `require_permission` authorization dependency, fail-closed audit logging, tenant-aware rate limiting, and a global 403 handler are implemented and verified against both a real local Postgres instance and a real hosted Supabase project — see `docs/rls-pattern.md` for how and why. Sprint 4 adds tenant-scoped CRUD for the company profile (fields on `organizations` itself), team members, skills, portfolio items, and case studies, plus a `files` table and signed-upload flow backed by a `StorageProvider` abstraction — verified end to end against a real Supabase project (real Auth sign-in, JWKS-verified JWTs, RLS-enforced tenant isolation, a real file upload). Sprint 5 adds `document_worker`/`embedding_worker` (Supabase Queues via `pgmq`), real PDF/DOCX/plain-text extraction, tenant- and permission/classification-filtered `pgvector` similarity search (`app/rag/`), and deletion propagation from files/portfolio items/case studies to their embedded chunks — also verified end to end against the real Supabase project, including a real worker crash-and-resume test proving no duplicate vectors on message redelivery. See `docs/rag-pattern.md` for why this needed a real *Supabase* project specifically (not just any real Postgres, unlike every prior sprint's RLS work). Sprint 6 (Opportunity Intake) is next.
