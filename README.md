...
Sprint 1, 1.5 (Design System), 2 (Identity & Tenancy Backend), and 3 (Authorization, RLS & Tenant Isolation Hardening) are complete. Cross-tenant row-level security, the `require_permission` authorization dependency, fail-closed audit logging, tenant-aware rate limiting, and a global 403 handler are implemented and verified against a real local Postgres instance (not just SQLite) — see `docs/rls-pattern.md` for how and why. Sprint 4 (Company Brain Data Model) is next.
